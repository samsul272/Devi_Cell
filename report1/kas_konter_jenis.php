...
<?php
$sql = $koneksi->query("SELECT SUM(masuk) as tot_masuk from transaksi where jenis='Masuk' and tgl_transaksi BETWEEN '$dt1' AND '$dt2'");
while ($data = $sql->fetch_assoc()) {
  $masuk = $data['tot_masuk'];
}

$sql = $koneksi->query("SELECT SUM(keluar) as tot_keluar from transaksi where jenis='Keluar' and tgl_transaksi BETWEEN '$dt1' AND '$dt2'");
while ($data = $sql->fetch_assoc()) {
      ...
      <table border="1" cellspacing="0">
        ...
        <tbody>
          ...
          if (isset($_POST["btnJenis"])) { {
              if ($jenis == 'Keluar') {
                $sql_tampil = "SELECT * FROM transaksi,kategori where id_kategori=uraian and tgl_transaksi BETWEEN '$dt1' AND '$dt2' and $jenis='Masuk' order by id_transaksi desc";
          ...
          ?>
                ...
                $sql_tampil = "SELECT * FROM transaksi,kategori where id_kategori=uraian and tgl_transaksi BETWEEN '$dt1' AND '$dt2' and $jenis='Keluar' order by id_transaksi desc";
                $query_tampil = mysqli_query($koneksi, $sql_tampil);
                $no = 1;
                while ($data = mysqli_fetch_array($query_tampil, MYSQLI_BOTH)) {
                ?>
